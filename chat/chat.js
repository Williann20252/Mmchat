...
function carregarMensagensFiltradas() {
  mural.innerHTML = "";
  onChildAdded(ref(db, "mensagens"), (snap) => {
    const msg = snap.val();
  if (msg.hora < loginTime) return;
    const participantesPV = pvAtivos[msg.uid] || [];

    if (modoPVAtivo && msg.tipo === "texto" && !participantesPV.includes(uid) && msg.uid !== uid) {
      return; // Oculta mensagens não privadas
    }
...
document.getElementById("alternarModoPV").onclick = () => {
  modoPVAtivo = !modoPVAtivo;
  mural.innerHTML = "";
  carregarMensagensFiltradas();
};
