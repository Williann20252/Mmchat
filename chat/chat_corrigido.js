// chat.js final com PV, botões e janelas flutuantes funcionando
...
const nickname = localStorage.getItem("nickname") || "Anônimo"; let uid = localStorage.getItem("uid"); if (!uid) { uid = "user_" + Math.random().toString(36).substring(2, 10); localStorage.setItem("uid", uid); }

let bloqueados = JSON.parse(localStorage.getItem("bloqueados") || "[]");

const mural = document.getElementById("chat-mural"); const input = document.getElementById("mensagemInput"); const enviarBtn = document.getElementById("enviarBtn"); const usuariosBtn = document.getElementById("usuariosBtn"); const configBtn = document.getElementById("configBtn"); const listaUsuarios = document.getElementById("listaUsuarios"); const logoutBtn = document.getElementById("logoutBtn"); const imgBtn = document.getElementById("imgBtn"); const audioBtn = document.getElementById("audioBtn"); const painelUsuarios = document.getElementById("usuariosOnline"); const painelConfig = document.getElementById("configPainel"); const fecharUsuarios = document.getElementById("fecharUsuarios");

const userRef = ref(db, "onlineUsers/" + uid); set(userRef, nickname); window.addEventListener("beforeunload", () => remove(userRef));

onValue(ref(db, "onlineUsers"), (snapshot) => { listaUsuarios.innerHTML = ""; const data = snapshot.val() || {}; Object.entries(data).forEach(([key, name]) => { const li = document.createElement("li"); if (key === uid) { li.textContent = ${name} (Você); } else { li.innerHTML = ${name} <button class="btn-pv" data-uid="${key}" data-nick="${name}" title="Falar no PV">🗨️</button> <button class="btn-bloquear" data-uid="${key}" title="Bloquear">🚫</button>; } listaUsuarios.appendChild(li); }); });
...
onChildAdded(ref(db, "pvSolicitacoes"), (snap) => { const s = snap.val(); if (s.paraUid === uid && s.status === "pendente") { const div = document.createElement("div"); div.className = "msg-pv"; div.innerHTML = <strong>@${s.deNick}</strong> deseja conversar reservadamente. <button class='aceitarPV' data-id='${snap.key}'>Aceitar</button>  <button class='recusarPV' data-id='${snap.key}'>Recusar</button>; mural.appendChild(div); } });
...
if (e.target.classList.contains("btn-pv")) { const destUid = e.target.dataset.uid; const destNick = e.target.dataset.nick; if (!bloqueados.includes(destUid)) { solicitarPV(destUid, destNick); } else { alert("Você bloqueou este usuário."); } }

if (e.target.classList.contains("btn-bloquear")) { const bloquearUid = e.target.dataset.uid; if (!bloqueados.includes(bloquearUid)) { bloqueados.push(bloquearUid); localStorage.setItem("bloqueados", JSON.stringify(bloqueados)); alert("Usuário bloqueado."); } else { alert("Usuário já está bloqueado."); } } });

function enviarMensagem() { const texto = input.value.trim(); if (!texto) return; push(ref(db, "mensagens"), { nick: nickname, uid: uid, tipo: "texto", conteudo: texto, hora: Date.now() }); input.value = ""; }
