...
// **Marcar início da sessão para filtrar histórico**
...
function enviarMensagem() {
  ...
  if (pvMode.checked) {
    ...
    if (!dest) {
      alert("Selecione usuário para PV.");
      return;
    }
...
// ————— Renderização de mensagens com filtro de histórico —————
onChildAdded(ref(db, "mensagens"), snap => {
  const msg = snap.val();

  // **Ignora todo msg.hora anterior ao início da sessão**
